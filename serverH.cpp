...
void createUDP(){
	...
	if ((rv = getaddrinfo(LOCAL_HOST, SERVERH, &hints, &servinfo)) != 0){
		exit(1);
	}	
...
int main(){
    ...
    while(true){
        ...
        cout<<"Server H finished sending the availability status of code "<<bookcode<<" to the Main Server using UDP on port "<<SERVERH<<'.'<<endl;
    }
